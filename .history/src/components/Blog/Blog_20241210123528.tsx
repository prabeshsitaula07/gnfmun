...
const BlogCard: React.FC<BlogCardProps> = ({ imgSrc, title, author, date, profileImg }) => (
    <div classNameName="gap-4 h-fit w-[25rem] flex flex-col mb-[3.75rem]">
        <img src={imgSrc} alt="blog" classNameName="border border-[#D9D9D9] w-full h-60 rounded-xl" />
        <h2 classNameName="font-medium text-2xl leading-10">{title}</h2>
        <div classNameName="flex h-9 w-72 gap-5 items-center">
            <img src={profileImg} alt="profile" classNameName="h-9 w-9 rounded-[28px]" />
            <h4 classNameName="font-medium text-base leading-6">{author}</h4>
            <h4 classNameName="font-normal text-base leading-6">{date}</h4>
        </div>
    </div>
);
