...
const Navbar = () => {
    ...
    return (
        ...
        <header className="px-6 md:px-14 my-10 fixed w-full bg-white z-50">
            {/* Contact Info */}
            <div
                className={`fixed top-0 left-0 w-full bg-primary px-6 py-3 text-center text-sm transition-transform duration-300 ${isScrolled ? "-translate-y-full" : "translate-y-0"
                    }`}
            >
            ...
            {/* navlinks */}
            <nav className={`flex justify-between items-center px-4 border-b $ {isScrolled }`}>
                <Link to="/">
                    <img src={logo} alt="" className="h-24 w-auto mx-auto p-3" />
                </Link>
                {/* Navbar Links */}
                <ul
