...
const Navbar = () => {
    ...
    return (
        <header className="px-6 md:px-14 my-10 fixed w-full bg-white z-50">
            {/* Contact Info */}
            <div
                className={`fixed top-0 left-0 w-full bg-primary px-6 py-4 text-center text-sm transition-transform duration-300 ${isScrolled ? "translate-y-0" : "-translate-y-full"
                    }`}
            >
                <span className="text-black font-semibold">Call us: +123 456 7890 | Email: info@example.com</span>
            </div>

            <nav className="flex justify-between items-center border border-gray-30 px-4">
                <a href="#">
